...
class teamCreate(commands.Cog):
    ...
    @app_commands.command(name="강제팀_등록", description="팀원을 등록 하세요")
    @app_commands.describe(team_title="팀명을 입력 해주세요.")
    @app_commands.describe(team_member_list="팀원들만 입력 해주세요.")
    async def adminTeamCreate(self, interaction: Interaction, team_title: str, team_member_list: str):
        ...
        if result == True:
            ...
            overwrites = {
                ...
                members[0]: PermissionOverwrite(read_messages=True, view_channel=True, send_messages=True),
